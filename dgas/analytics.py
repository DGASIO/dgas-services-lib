...
def encode_id(dgas_id):
    return sha256(dgas_id.encode('utf-8')).hexdigest() if dgas_id else None

class AnalyticsMixin:

    def track(self, dgas_id, event, data=None, add_user_agent=True):
        if add_user_agent and (data is None or 'User-Agent' not in data):
    ...
    def people_set(self, dgas_id, data=None):
        if self.application.mixpanel_instance:
            self.application.mixpanel_instance.people_set(encode_id(dgas_id), data)
