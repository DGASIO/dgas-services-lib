...
class TokenHTTPClientBase:

    def fetch(self, url, method="GET", body=None, headers=None, timeout=None):
        if not isinstance(url, TokenHTTPRequest):
            req = TokenHTTPRequest(url, method=method, body=body, headers=headers, timeout=timeout)
        else:
            req = url
